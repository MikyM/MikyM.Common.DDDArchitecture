...
{
    public abstract class CrudService<TEntity, T, TContext> : ReadOnlyService<TEntity, T, TContext>, ICrudService<TEntity, T> where TEntity : AggregateRootEntity where T : class where TContext : DbContext
    {
        ...
        {
            _unitOfWork.GetRepository<TEntity, Repository<TEntity>>().Update(_mapper.Map<TEntity>(updateObject));
            if (shouldSave)
                await CommitAsync();
            return true;
            //to do
        ...
        {
            _unitOfWork.GetRepository<TEntity, Repository<TEntity>>().UpdateRange(_mapper.Map<IEnumerable<TEntity>>(updateObjects));
            if (shouldSave)
                await CommitAsync();
            return true;
            //to do
        ...
        {
            _unitOfWork.GetRepository<TEntity, Repository<TEntity>>().Delete(_mapper.Map<TEntity>(deleteObject));
        ...
        {
            _unitOfWork.GetRepository<TEntity, Repository<TEntity>>().DeleteRange(deleteObjects);
